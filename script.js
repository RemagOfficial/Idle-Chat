...
const GENERATOR_COSTS = {
    ...
    prestige: 1000000 // 10x generator3
};
...
let prestigeUpdateInterval = null; // Interval for updating prestige channel display
...
// Game state
let gameState = {
    messages: new Decimal(0), // Decimal for message count
    fractionalMessages: 0, // Accumulated fractional messages (kept as number, always < 1)
    lifetimeMessages: new Decimal(0), // Total messages ever generated (including spent) - Decimal
    ...
    settings: {
        ...
        useMobileButton: true // Use mobile button instead of text input on mobile
    },
    upgrades: {
    ...
    generator1BotMessages: new Decimal(0), // Total messages generated by generator1 bots (for pet photo tracking) - Decimal
    ...
    prestigeBoost: 1.0, // Full game prestige multiplier (linear: 1% per 1T messages, compounds when prestiging)
    lastPrestigeMessageCount: new Decimal(0) // Message count at the time of last prestige (Decimal to handle large numbers)
};
...
function loadSettings() {
    ...
    // Ensure mobile button setting exists with default (defaults to true)
    if (gameState.settings.useMobileButton === undefined) {
        gameState.settings.useMobileButton = true;
    }
}
...
const servers = {
    ...
    prestige: {
        ...
        locked: true,
        channels: {
            prestige: { name: 'prestige', content: 'Full game prestige and reset' }
        }
    }
};
...
function getGeneratorServerName(generatorId) {
    ...
    // Check if generator has a stored name (preserved even when locked)
    if (gameState.generators[generatorId]?.name) {
...
function getServerPingCount(serverId) {
    ...
    // Prestige: show "!" if it can be unlocked (requires generator3)
    if (serverId === 'prestige') {
        if (!gameState.generators.unlocked.includes('prestige') && isGeneratorUnlocked('generator3')) {
            const unlockCost = GENERATOR_COSTS.prestige;
            if (totalMessages >= unlockCost) {
                return '!'; // Can be unlocked
            }
            return null; // Can't unlock yet
    ...
    // Unlocked generators: count affordable upgrades (respects bulk purchase amount)
    if (serverId === 'generator1') {
        ...
        if (gen.bots >= 1 && gen.bots < maxLevels.bots) {
            ...
            const actualCount = Math.min(bulkPurchaseAmount, maxLevels.bots - gen.bots);
            if (actualCount > 0) {
        ...
        if (efficiencyLevel < maxLevels.efficiency) {
            const actualCount = Math.min(bulkPurchaseAmount, maxLevels.efficiency - efficiencyLevel);
            if (actualCount > 0) {
        ...
        if (botSpeedLevel < maxLevels.botSpeed) {
            const actualCount = Math.min(bulkPurchaseAmount, maxLevels.botSpeed - botSpeedLevel);
            if (actualCount > 0) {
                const botSpeedCost = getBulkBotSpeedCost(botSpeedLevel, bulkPurchaseAmount, maxLevels.botSpeed);
        ...
        if (gen.autoBuyPurchased === true) {
            ...
            if (autoBuyDelayLevel < maxDelayLevel) {
                const actualCount = Math.min(bulkPurchaseAmount, maxDelayLevel - autoBuyDelayLevel);
                if (actualCount > 0) {
                    const autoBuyDelayCost = getBulkAutoBuyDelayCost(autoBuyDelayLevel, bulkPurchaseAmount, maxDelayLevel);
    ...
    if (serverId === 'generator2') {
        ...
        if ((gen2.cascades || 0) < maxLevels.cascades) {
            ...
            if (hasGen1Bot) {
                ...
                if (actualCount > 0) {
                    const cascadeCost = getBulkCascadeCost(currentCascades, bulkPurchaseAmount, maxLevels.cascades);
    ...
    if (serverId === 'upgrades') {
        ...
        const GLOBAL_MAX = getGlobalUpgradeMaxLevels();
        
        // Manual generation multiplier (max 25)
        const manualMultLevel = upgrades.manualGenerationMultiplier || 0;
        if (manualMultLevel < GLOBAL_MAX.manualGenerationMultiplier) {
            const actualCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.manualGenerationMultiplier - manualMultLevel);
            if (actualCount > 0) {
                const cost = getBulkManualGenerationUpgradeCost(manualMultLevel, bulkPurchaseAmount, GLOBAL_MAX.manualGenerationMultiplier);
        ...
        if (messageMultLevel < GLOBAL_MAX.messageMultiplier) {
            const actualCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.messageMultiplier - messageMultLevel);
            if (actualCount > 0) {
                const cost = getBulkMessageMultiplierCost(messageMultLevel, bulkPurchaseAmount, GLOBAL_MAX.messageMultiplier);
                if (totalMessages >= cost) count++;
            }
        }
        ...
        if (autoGenBoostLevel < GLOBAL_MAX.autoGenerationBoost) {
            const actualCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.autoGenerationBoost - autoGenBoostLevel);
            if (actualCount > 0) {
                const cost = getBulkAutoGenerationBoostCost(autoGenBoostLevel, bulkPurchaseAmount, GLOBAL_MAX.autoGenerationBoost);
                if (totalMessages >= cost) count++;
            }
        }
        ...
        if (costEfficiencyLevel < GLOBAL_MAX.costEfficiency) {
            const actualCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.costEfficiency - costEfficiencyLevel);
            if (actualCount > 0) {
                const cost = getBulkCostEfficiencyCost(costEfficiencyLevel, bulkPurchaseAmount, GLOBAL_MAX.costEfficiency);
                if (totalMessages >= cost) count++;
            }
        }
...
// Get total ping count across all servers
function getTotalPingCount() {
    let total = 0;
    
    // Check all possible servers
    const serversToCheck = ['home', 'generator1', 'generator2', 'generator3', 'prestige', 'upgrades'];
    
    serversToCheck.forEach(serverId => {
        const pingCount = getServerPingCount(serverId);
        if (pingCount) {
            // If pingCount is a number, add it; if it's '!', add 1
            if (typeof pingCount === 'number') {
                total += pingCount;
            } else if (pingCount === '!') {
                total += 1;
            }
...
function updateServerPings() {
    ...
    // Update hamburger menu button ping
    const menuButton = document.getElementById('mobile-menu-button');
    if (menuButton) {
        const totalPings = getTotalPingCount();
        let menuPingBadge = menuButton.querySelector('.mobile-menu-ping');
        
        if (totalPings) {
            if (!menuPingBadge) {
                menuPingBadge = document.createElement('div');
                menuPingBadge.className = 'mobile-menu-ping';
                menuButton.appendChild(menuPingBadge);
            }
            menuPingBadge.textContent = totalPings;
        } else {
            if (menuPingBadge) {
                menuPingBadge.remove();
            }
        }
    }
}
...
function renderServerSidebar() {
    ...
    // Show prestige if generator3 is unlocked
    if (isGeneratorUnlocked('generator3')) {
        const prestigeUnlocked = gameState.generators.unlocked.includes('prestige');
        const prestigeIcon = createServerIcon('prestige', 'P', false, !prestigeUnlocked);
        sidebar.appendChild(prestigeIcon);
    }
...
function unlockGenerator(generatorId) {
    ...
    const isUnlocked = generatorId === 'prestige'
        ? gameState.generators.unlocked.includes('prestige')
        : isGeneratorUnlocked(generatorId);
    if (isUnlocked) return false;
    
    // Prestige requires generator3 to be unlocked first
    if (generatorId === 'prestige' && !isGeneratorUnlocked('generator3')) {
        return false;
    }
    ...
    if (totalMessages >= cost) {
        ...
        if (gameState.fractionalMessages >= cost) {
            ...
            gameState.messages = gameState.messages.minus(Math.ceil(remaining));
        }
        
        // Unlock generator/prestige
...
function getGeneratorProduction(generatorId) {
    ...
    if (generatorId === 'generator1') {
        ...
        baseProduction = baseProduction * getResearchBotMultiplier();
        
        // Apply prestige boost
        baseProduction = baseProduction * getPrestigeBoost();
        ...
        // Apply research multiplier
        baseProduction = baseProduction * getResearchCascadeMultiplier();
...
// Get prestige boost multiplier (compounding boost from prestiges)
function getPrestigeBoost() {
    return gameState.prestigeBoost || 1.0;
}

// Calculate prestige boost for a given message count
// Formula: 1% per 1T messages (linear), so 1T messages = +1% = 1.01x multiplier
function getPrestigeBoostForMessages(messageCount) {
    // Handle very large numbers to prevent overflow
    if (!isFinite(messageCount) || messageCount <= 0) {
        return 1.0;
    }
    
    // Linear formula: 1% per 1T messages
    // boost = 1 + (messages / 1,000,000,000,000) * 0.01
    // Or: boost = 1 + (messages / 100,000,000,000,000)
    // So 1T messages = 1 + (1T / 100T) = 1 + 0.01 = 1.01 = +1%
    const boostMultiplier = 1.0 + (messageCount / 100000000000000);
    
    // Check if the result is too large for JavaScript
    if (!isFinite(boostMultiplier) || boostMultiplier === Infinity) {
        // For extremely large values, return a marker with the multiplier value
        // We can still represent it as: 1 + (messages / 100T)
        return { isVeryLarge: true, multiplier: messageCount / 100000000000000 };
    }
...
function init() {
    ...
    setInterval(() => {
        ...
        if (totalProduction > 0) {
            ...
            if (wholePart > 0) {
                gameState.messages = gameState.messages.plus(wholePart);
                gameState.lifetimeMessages = (gameState.lifetimeMessages || new Decimal(0)).plus(wholePart);
        ...
        if (generator1Production > 0 && isGeneratorUnlocked('generator1')) {
            if (!gameState.generator1BotMessages) {
                gameState.generator1BotMessages = new Decimal(0);
            }
            ...
            if (gen1WholePart > 0) {
                const previousCount = gameState.generator1BotMessages || new Decimal(0);
                gameState.generator1BotMessages = previousCount.plus(gen1WholePart);
                ...
                const previousHundreds = previousCount.dividedBy(100).toNumber();
                const currentHundreds = gameState.generator1BotMessages.dividedBy(100).toNumber();
...
function getTotalMessagesPerSecond() {
    ...
    // Add actual manual generation rate (already includes both multipliers and prestige boost)
    total += getManualGenerationRate();
    
    // Prestige boost is already applied in getGeneratorProduction() and getManualGenerationRate()
    return total;
}
...
function updateUpgradeButtonStates() {
    ...
    if (currentServer === 'generator1' && currentChannel === 'general') {
        ...
        if (manualMultiplierBtn) {
            ...
            const isMaxLevel = multiplierLevel >= getGlobalUpgradeMaxLevels().manualGenerationMultiplier;
            if (isMaxLevel) {
                ...
                const actualCount = Math.min(bulkPurchaseAmount, getGlobalUpgradeMaxLevels().manualGenerationMultiplier - multiplierLevel);
                const upgradeCost = getBulkManualGenerationUpgradeCost(multiplierLevel, bulkPurchaseAmount, getGlobalUpgradeMaxLevels().manualGenerationMultiplier);
        ...
        if (autoBoostBtn) {
            const autoBoostLevel = gameState.upgrades.autoGenerationBoost || 0;
            const isMaxLevel = autoBoostLevel >= getGlobalUpgradeMaxLevels().autoGenerationBoost;
            if (isMaxLevel) {
                ...
                const actualCount = Math.min(bulkPurchaseAmount, getGlobalUpgradeMaxLevels().autoGenerationBoost - autoBoostLevel);
                const autoBoostCost = getBulkAutoGenerationBoostCost(autoBoostLevel, bulkPurchaseAmount, getGlobalUpgradeMaxLevels().autoGenerationBoost);
        ...
        if (messageMultiplierBtn) {
            const messageMultiplierLevel = gameState.upgrades.messageMultiplier || 0;
            const isMaxLevel = messageMultiplierLevel >= getGlobalUpgradeMaxLevels().messageMultiplier;
            if (isMaxLevel) {
                ...
                const actualCount = Math.min(bulkPurchaseAmount, getGlobalUpgradeMaxLevels().messageMultiplier - messageMultiplierLevel);
                const messageMultiplierCost = getBulkMessageMultiplierCost(messageMultiplierLevel, bulkPurchaseAmount, getGlobalUpgradeMaxLevels().messageMultiplier);
        ...
        if (costEfficiencyBtn) {
            ...
            const isMaxLevel = costEfficiencyLevel >= getGlobalUpgradeMaxLevels().costEfficiency;
            if (isMaxLevel) {
                ...
                const actualCount = Math.min(bulkPurchaseAmount, getGlobalUpgradeMaxLevels().costEfficiency - costEfficiencyLevel);
                const costEfficiencyCost = getBulkCostEfficiencyCost(costEfficiencyLevel, bulkPurchaseAmount, getGlobalUpgradeMaxLevels().costEfficiency);
...
function getTotalMessagesAsNumber() {
    // Convert Decimal to number, handling very large values
    if (gameState.messages instanceof Decimal) {
        const numValue = gameState.messages.toNumber();
        if (!isFinite(numValue)) {
            return Number.MAX_SAFE_INTEGER;
        }
        return numValue + (gameState.fractionalMessages || 0);
    }
    // Fallback for non-Decimal (shouldn't happen)
    return Number(gameState.messages || 0) + (gameState.fractionalMessages || 0);
}

// Helper function to get total messages for display (returns Decimal for formatNumber)
function getTotalMessagesForDisplay() {
    // Return the Decimal plus fractional part - formatNumber will handle it properly
    if (gameState.messages instanceof Decimal) {
        const fractional = gameState.fractionalMessages || 0;
        if (fractional > 0) {
            return gameState.messages.plus(fractional);
        }
        return gameState.messages;
    }
    // Fallback for non-Decimal (shouldn't happen)
    return new Decimal(gameState.messages || 0).plus(gameState.fractionalMessages || 0);
}

// Update currency display
function updateCurrencyDisplay() {
    const totalMessages = getTotalMessagesForDisplay(); // Use Decimal for display
...
function formatNumber(num, decimals = 0) {
    // Handle Decimal
    const isDecimal = num instanceof Decimal;
    let decimalValue;
    let numValue;
    
    if (isDecimal) {
        decimalValue = num;
        const converted = num.toNumber();
        numValue = isFinite(converted) ? converted : null;
    } else {
        numValue = num;
        decimalValue = new Decimal(num || 0);
    }
    
    // Zero is always just "0"
    // Check if the value is zero - try Decimal methods first, then fall back to number comparison
    try {
        if (decimalValue instanceof Decimal) {
            // Try different possible method names for checking zero
            if (typeof decimalValue.equals === 'function' && decimalValue.equals(0)) return '0';
            if (typeof decimalValue.eq === 'function' && decimalValue.eq(0)) return '0';
            if (typeof decimalValue.isZero === 'function' && decimalValue.isZero()) return '0';
            // Fallback: convert to number
            const num = decimalValue.toNumber();
            if (num === 0) return '0';
        }
    } catch (e) {
        // If Decimal methods fail, fall back to number comparison
    }
    // Final fallback: check the number value directly
    ...
    if (format === 'abbreviated') {
        ...
        // Calculate order (tier) from Decimal
        let order;
        if (decimalValue.lessThan(thousand)) {
            return formatWithDecimals(numValue || 0, decimals);
        }
        
        // Calculate order using Decimal logarithm
        let log10Value;
        try {
            if (typeof decimalValue.log10 === 'function') {
                const log10Result = decimalValue.log10();
                // log10 might return a Decimal or a number
                if (log10Result instanceof Decimal) {
                    log10Value = log10Result.toNumber();
                } else if (typeof log10Result === 'number') {
                    log10Value = log10Result;
                } else {
                    // Fallback: use Math.log10 on the number value
                    log10Value = Math.log10(numValue || 1);
                }
            } else {
                // Fallback: use Math.log10
                log10Value = Math.log10(numValue || 1);
            }
        } catch (e) {
            // Fallback: use Math.log10
        ...
        if (suffixIndex === 0) {
            // For numbers less than 1000, format with proper decimals
            const formatted = decimalValue.toFixed(decimals);
        ...
        // Divide using Decimal to maintain precision
        const divisor = thousand.pow(suffixIndex);
        const divided = decimalValue.dividedBy(divisor);
        // Use the decimals parameter instead of hardcoded 2
        const value = divided.toFixed(decimals);
        ...
        // Use Decimal for scientific notation
        let log10Value;
        try {
            if (typeof decimalValue.log10 === 'function') {
                const log10Result = decimalValue.log10();
        ...
        const exponent = Math.floor(log10Value);
        const mantissa = decimalValue.dividedBy(new Decimal(10).pow(exponent));
        const mantissaStr = mantissa.toFixed(2);
        const formattedExponent = exponent.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        return `${mantissaStr}e+${formattedExponent}`;
    } else {
        // Full format with commas and decimals
        const numStr = decimalValue.toFixed(decimals);
...
function loadServer(serverId) {
    ...
    if (servers[serverId].locked) {
        // Prestige requires generator3 to be unlocked first
        if (serverId === 'prestige' && !isGeneratorUnlocked('generator3')) {
            // Show message that generator3 must be unlocked first
            const contentBody = document.getElementById('content-body');
            contentBody.innerHTML = `
                <div class="unlock-prompt">
                    <div class="unlock-content">
                        <h2>🔒 Prestige</h2>
                        <p>You must unlock Generator 3 (Research Lab) before accessing Prestige.</p>
                    </div>
                </div>
            `;
            return;
        ...
        // Check if unlocked (generators use isGeneratorUnlocked, prestige uses separate check)
        const isUnlocked = serverId === 'prestige' 
            ? gameState.generators.unlocked.includes('prestige')
            : isGeneratorUnlocked(serverId);
...
function loadChannel(channelId) {
    ...
    // Remove mobile buttons if switching away from their respective channels
    if (!(currentServer === 'home' && channelId === 'manual')) {
        const existingMobileButton = document.getElementById('mobile-generate-button');
    ...
    if (!(currentServer === 'home' && channelId === 'remagofficial')) {
        const existingDMMobileButton = document.getElementById('dm-mobile-generate-button');
        if (existingDMMobileButton) {
            existingDMMobileButton.remove();
        }
    }
    ...
    if (channelId === 'manual' && currentServer === 'home') {
        const manualMultiplier = getManualGenerationMultiplier();
        const globalMultiplier = getGlobalMessageMultiplier();
        const prestigeBoost = getPrestigeBoost();
        const totalMultiplier = manualMultiplier * globalMultiplier * prestigeBoost;
        ...
        // Remove any existing mobile button
        const existingMobileButton = document.getElementById('mobile-generate-button');
        if (existingMobileButton) {
            existingMobileButton.remove();
        }
        
        // Apply setting to body class for CSS
        const useMobileButton = gameState.settings.useMobileButton !== false; // Default to true
        if (window.innerWidth <= 768) {
            if (useMobileButton) {
                document.body.classList.remove('use-text-input');
            } else {
                document.body.classList.add('use-text-input');
            }
            
            // Create and add mobile generate button only if setting is enabled
            if (useMobileButton) {
                const mobileButton = document.createElement('button');
                mobileButton.className = 'mobile-generate-button';
                mobileButton.id = 'mobile-generate-button';
                mobileButton.innerHTML = `
                    <svg class="mobile-generate-icon" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/>
                    </svg>
                    <span class="mobile-generate-label">Tap</span>
                    <span class="mobile-generate-subtitle">or Hold</span>
                `;
        ...
        contentBody.innerHTML = `
            <div class="bulk-purchase-selector">
                <span class="bulk-purchase-label">Buy:</span>
                <div class="bulk-purchase-buttons" data-selected="${bulkPurchaseAmount}">
                    ${[1, 5, 10, 25, 50].map(num => `
                        <button class="bulk-purchase-btn ${bulkPurchaseAmount === num ? 'active' : ''}" data-amount="${num}">${num}</button>
                    `).join('')}
                </div>
            </div>
        ...
        const bulkPurchaseContainer = document.querySelector('.bulk-purchase-buttons');
        bulkPurchaseBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                bulkPurchaseAmount = parseInt(btn.dataset.amount);
                if (bulkPurchaseContainer) {
                    bulkPurchaseContainer.setAttribute('data-selected', bulkPurchaseAmount);
                }
        ...
        if (unlockPetPhotosBtn) {
            unlockPetPhotosBtn.addEventListener('click', () => {
                ...
                if (researchPoints >= petPhotosCost && isMaxBot && !petPhotosUnlocked) {
                    ...
                    const cost = new Decimal(petPhotosCost * 10000);
                    gameState.messages = gameState.messages.minus(cost); // Deduct research points (1 RP = 10,000 messages)
                    if (gameState.messages.lessThan(0)) {
                        gameState.fractionalMessages += gameState.messages.toNumber();
                        gameState.messages = new Decimal(0);
                    }
                    autoSave();
                    updateCurrencyDisplay();
        ...
        if (unlockOffTopicBtn) {
            unlockOffTopicBtn.addEventListener('click', () => {
                ...
                if (researchPoints >= offTopicCost && petPhotosUnlocked && !offTopicUnlocked) {
                    ...
                    const cost = new Decimal(offTopicCost * 10000);
                    gameState.messages = gameState.messages.minus(cost);
                    checkAchievements();
                    if (gameState.messages.lessThan(0)) {
                        gameState.fractionalMessages += gameState.messages.toNumber();
                        gameState.messages = new Decimal(0);
        ...
        if (unlockTechBtn) {
            unlockTechBtn.addEventListener('click', () => {
                ...
                if (researchPoints >= techCost && isMaxCascade && !techUnlocked) {
                    ...
                    const cost = new Decimal(techCost * 10000);
                    gameState.messages = gameState.messages.minus(cost);
                    checkAchievements();
                    if (gameState.messages.lessThan(0)) {
                        gameState.fractionalMessages += gameState.messages.toNumber();
        ...
        if (unlockScienceBtn) {
            unlockScienceBtn.addEventListener('click', () => {
                ...
                if (researchPoints >= scienceCost && isMaxManual && !scienceUnlocked) {
                    ...
                    const cost = new Decimal(scienceCost * 10000);
                    gameState.messages = gameState.messages.minus(cost);
                    checkAchievements();
                    if (gameState.messages.lessThan(0)) {
                        gameState.fractionalMessages += gameState.messages.toNumber();
        ...
        const multiplierLevel = gameState.upgrades.manualGenerationMultiplier || 0;
        const GLOBAL_MAX = getGlobalUpgradeMaxLevels();
        const isMaxManualMultiplier = multiplierLevel >= GLOBAL_MAX.manualGenerationMultiplier;
        ...
        const autoBoostLevel = gameState.upgrades.autoGenerationBoost || 0;
        const isMaxAutoBoost = autoBoostLevel >= GLOBAL_MAX.autoGenerationBoost;
        ...
        const messageMultiplierLevel = gameState.upgrades.messageMultiplier || 0;
        const isMaxMessageMultiplier = messageMultiplierLevel >= GLOBAL_MAX.messageMultiplier;
        ...
        const costEfficiencyLevel = gameState.upgrades.costEfficiency || 0;
        const isMaxCostEfficiency = costEfficiencyLevel >= GLOBAL_MAX.costEfficiency;
        ...
        const actualManualMultiplierCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.manualGenerationMultiplier - multiplierLevel);
        const actualAutoBoostCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.autoGenerationBoost - autoBoostLevel);
        const actualMessageMultiplierCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.messageMultiplier - messageMultiplierLevel);
        const actualCostEfficiencyCount = Math.min(bulkPurchaseAmount, GLOBAL_MAX.costEfficiency - costEfficiencyLevel);
        
        const bulkManualMultiplierCost = getBulkManualGenerationUpgradeCost(multiplierLevel, bulkPurchaseAmount, GLOBAL_MAX.manualGenerationMultiplier);
        const bulkAutoBoostCost = getBulkAutoGenerationBoostCost(autoBoostLevel, bulkPurchaseAmount, GLOBAL_MAX.autoGenerationBoost);
        const bulkMessageMultiplierCost = getBulkMessageMultiplierCost(messageMultiplierLevel, bulkPurchaseAmount, GLOBAL_MAX.messageMultiplier);
        const bulkCostEfficiencyCost = getBulkCostEfficiencyCost(costEfficiencyLevel, bulkPurchaseAmount, GLOBAL_MAX.costEfficiency);
        ...
        contentBody.innerHTML = `
            <div class="settings-content">
                ...
                <div class="settings-section">
                    <h3 class="settings-title">Mobile Controls</h3>
                    <p class="settings-description">Choose how to generate messages manually on mobile devices.</p>
                    <div class="settings-options">
                        <label class="settings-option">
                            <input type="radio" name="mobileInput" value="button" ${gameState.settings.useMobileButton !== false ? 'checked' : ''}>
                            <span>Use Mobile Button (Recommended)</span>
                        </label>
                        <label class="settings-option">
                            <input type="radio" name="mobileInput" value="text" ${gameState.settings.useMobileButton === false ? 'checked' : ''}>
            ...
            <div class="stats-content">
                ...
                <div class="stats-section">
                    ...
                    <div class="stats-grid">
                        ...
                        ` : ''}
                        ${gameState.prestigeBoost > 1.0 ? `
                        <div class="stat-item">
                            <span class="stat-label">Prestige Boost:</span>
                            <span class="stat-value">+${formatNumber((gameState.prestigeBoost - 1.0) * 100, 2)}%</span>
                        </div>
                        ` : ''}
                        ${manualGenLevel === 0 && autoBoostLevel === 0 && messageMultiplierLevel === 0 && costEfficiencyLevel === 0 && gameState.prestigeBoost <= 1.0 ? `
                        <div class="stat-item">
        ...
        renderAchievements();
    } else if (channelId === 'prestige' && currentServer === 'prestige') {
        // Prestige channel
        const totalMessages = getTotalMessagesAsNumber();
        const currentBoost = getPrestigeBoost();
        const boostPercent = formatNumber((currentBoost - 1.0) * 100, 2);
        
        // Calculate what the boost would be after prestiging
        const newBoostFromMessages = getPrestigeBoostForMessages(totalMessages);
        
        // Calculate additional boost and extra global upgrade levels
        let additionalBoostPercent = 0;
        let extraLevels = 0;
        let totalBoostAfterPrestige = currentBoost;
        
        // Check if we got a very large value marker
        if (newBoostFromMessages && typeof newBoostFromMessages === 'object' && newBoostFromMessages.isVeryLarge) {
            const newBoostValue = 1.0 + newBoostFromMessages.multiplier;
            totalBoostAfterPrestige = currentBoost * newBoostValue;
            const currentWholePercent = Math.floor((currentBoost - 1.0) * 100);
            const afterWholePercent = Math.floor((totalBoostAfterPrestige - 1.0) * 100);
            additionalBoostPercent = afterWholePercent - currentWholePercent;
            extraLevels = additionalBoostPercent * 10;
        } else {
            totalBoostAfterPrestige = currentBoost * newBoostFromMessages;
            if (isFinite(totalBoostAfterPrestige) && totalBoostAfterPrestige !== Infinity) {
                const currentWholePercent = Math.floor((currentBoost - 1.0) * 100);
                const afterWholePercent = Math.floor((totalBoostAfterPrestige - 1.0) * 100);
                additionalBoostPercent = afterWholePercent - currentWholePercent;
                extraLevels = additionalBoostPercent * 10;
        ...
        // Check if prestige would provide any benefit
        // Prestige should be disabled if:
        // 1. Boost after prestige is equal to or less than current boost, AND
        // 2. Extra levels are equal to or less than 0
        // 3. Current messages are less than the last prestige message count
        const currentMaxLevels = getGlobalUpgradeMaxLevels();
        const wouldGetExtraLevels = extraLevels > 0;
        const wouldGetBoostIncrease = totalBoostAfterPrestige > currentBoost;
        const lastPrestigeMessages = gameState.lastPrestigeMessageCount || new Decimal(0);
        const totalMessagesDecimal = new Decimal(Math.floor(totalMessages)) || new Decimal(0);
        const hasEnoughMessages = totalMessagesDecimal.greaterThanOrEqualTo(lastPrestigeMessages);
        const canPrestige = (wouldGetBoostIncrease || wouldGetExtraLevels) && hasEnoughMessages;
        
        // Format the boost after prestige percentage
        let boostAfterPrestigeText;
        
        // Check if we got a very large value marker
        if (newBoostFromMessages && typeof newBoostFromMessages === 'object' && newBoostFromMessages.isVeryLarge) {
            // Linear formula: newBoost = 1 + multiplier
            // Total boost = currentBoost * (1 + multiplier)
            // Percentage = (totalBoost - 1) * 100
            const newBoostValue = 1.0 + newBoostFromMessages.multiplier;
            const totalBoostAfterPrestige = currentBoost * newBoostValue;
            
            // Calculate percentage
            const boostAfterPercent = (totalBoostAfterPrestige - 1.0) * 100;
            
            // Format in scientific notation if needed
            if (!isFinite(boostAfterPercent) || boostAfterPercent > 1e15) {
                // Use log10 for very large values
                const log10TotalBoost = Math.log10(currentBoost) + Math.log10(newBoostValue);
                const log10Percent = log10TotalBoost + Math.log10(100);
                const exponent = Math.floor(log10Percent);
                const mantissa = Math.pow(10, log10Percent - exponent);
                boostAfterPrestigeText = `+${mantissa.toFixed(2)}e+${exponent}%`;
            } else {
                boostAfterPrestigeText = `+${formatNumber(boostAfterPercent, 2)}%`;
            }
            } else {
                const totalBoostAfterPrestige = currentBoost * newBoostFromMessages;
                
                if (!isFinite(totalBoostAfterPrestige) || totalBoostAfterPrestige === Infinity) {
                    // Fallback: calculate using linear formula (1% per 1T messages)
                    const multiplier = totalMessages / 100000000000000;
                    const newBoostValue = 1.0 + multiplier;
                    const totalBoost = currentBoost * newBoostValue;
                    const boostAfterPercent = (totalBoost - 1.0) * 100;
                    
                    if (!isFinite(boostAfterPercent)) {
                        // Use log10 for extremely large values
                        const log10TotalBoost = Math.log10(currentBoost) + Math.log10(newBoostValue);
                        const log10Percent = log10TotalBoost + Math.log10(100);
                        const exponent = Math.floor(log10Percent);
                        const mantissa = Math.pow(10, log10Percent - exponent);
                    ...
                    const boostAfterPercent = (totalBoostAfterPrestige - 1.0) * 100;
                    // For very large percentages, formatNumber should handle scientific notation
                    boostAfterPrestigeText = `+${formatNumber(boostAfterPercent, 2)}%`;
                }
            }
        ...
        contentBody.innerHTML = `
            <div class="settings-content">
                <div class="settings-section">
                    <h3 class="settings-title">Full Game Prestige</h3>
                    <p class="settings-description">Reset your progress to gain a permanent production boost. The boost is calculated based on your current message count at the time of prestiging.</p>
                    <div class="upgrade-stats" style="margin-top: 16px;">
                        <div class="upgrade-stat">
                            <span class="stat-label">Current Messages:</span>
                            <span class="stat-value" id="prestige-current-messages">${formatNumber(totalMessages, 2)}</span>
                        </div>
                        <div class="upgrade-stat">
                            <span class="stat-label">Current Boost:</span>
                            <span class="stat-value" id="prestige-current-boost">+${boostPercent}%</span>
                        </div>
                        <div class="upgrade-stat">
                            <span class="stat-label">Boost After Prestige:</span>
                            <span class="stat-value" id="prestige-boost-after">${boostAfterPrestigeText}</span>
                        </div>
                        ${extraLevels > 0 ? `
                        <div class="upgrade-stat">
                            <span class="stat-label">Extra Global Upgrade Levels:</span>
                            <span class="stat-value" id="prestige-extra-levels">+${formatNumber(extraLevels, 0)}</span>
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div class="settings-section">
                    <h3 class="settings-title">What Gets Reset</h3>
                    <ul style="color: var(--text-secondary); line-height: 1.8; padding-left: 20px;">
                        <li>All messages (reset to 0)</li>
                        <li>All upgrades (manual generation, auto boost, message multiplier, cost efficiency)</li>
                        <li>All research upgrades</li>
                        <li>All generator upgrades (bots, cascades, efficiency, speed, etc.)</li>
                        <li>All generator prestige levels</li>
                    </ul>
                </div>
                
                <div class="settings-section">
                    <h3 class="settings-title">What Stays</h3>
                    <ul style="color: var(--text-secondary); line-height: 1.8; padding-left: 20px;">
                        <li>DM progress (characters typed and reward status)</li>
                        <li>DM buffs (message multiplier bonus)</li>
                        <li>Prestige boost (permanent production multiplier)</li>
                    </ul>
                </div>
                
                <div class="settings-section">
                    <button class="settings-button danger ${!canPrestige ? 'disabled' : ''}" id="prestige-button" style="width: 100%; padding: 16px; font-size: 18px; font-weight: 600;" ${!canPrestige ? 'disabled' : ''}>
                        ${canPrestige ? 'Hold to Prestige (3 seconds)' : 'No Benefit Available'}
                    </button>
                    ${!canPrestige ? `
                    <p style="color: var(--text-muted); margin-top: 12px; text-align: center; font-size: 14px;">
                        ${!hasEnoughMessages ? `You need at least ${formatNumber(Number(lastPrestigeMessages), 2)} messages to prestige (you have ${formatNumber(totalMessages, 2)}).` : 'You need more messages to gain a benefit from prestiging.'}
                    </p>
                    ` : ''}
                    <div id="prestige-progress" style="margin-top: 12px; display: none;">
                        <div style="background-color: var(--bg-dark); border-radius: 4px; height: 8px; overflow: hidden;">
                            <div id="prestige-progress-bar" style="background-color: var(--accent-color); height: 100%; width: 0%; transition: width 0.1s linear;"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        const progressBar = document.getElementById('prestige-progress');
        const progressBarFill = document.getElementById('prestige-progress-bar');
        let holdTimeout = null;
        let progressInterval = null;
        let isHolding = false;
        
        if (prestigeBtn) {
            // Disable button if no benefit
            if (!canPrestige) {
            ...
            const startHold = (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (!canPrestige) return; // Don't allow holding if no benefit
                isHolding = true;
            ...
            prestigeBtn.addEventListener('mousedown', startHold);
            prestigeBtn.addEventListener('touchstart', startHold, { passive: false });
            prestigeBtn.addEventListener('mouseup', stopHold);
            prestigeBtn.addEventListener('mouseleave', stopHold);
            prestigeBtn.addEventListener('touchend', stopHold);
            prestigeBtn.addEventListener('touchcancel', stopHold);
        }
        
        // Update prestige display every second
        if (prestigeUpdateInterval) {
            clearInterval(prestigeUpdateInterval);
        }
        prestigeUpdateInterval = setInterval(() => {
            const currentMessagesEl = document.getElementById('prestige-current-messages');
            const currentBoostEl = document.getElementById('prestige-current-boost');
            const boostAfterEl = document.getElementById('prestige-boost-after');
            
            if (currentMessagesEl || currentBoostEl || boostAfterEl) {
                const totalMessages = getTotalMessagesAsNumber();
                const currentBoost = getPrestigeBoost();
                const boostPercent = formatNumber((currentBoost - 1.0) * 100, 2);
                ...
                if (prestigeBtn) {
                    if (!canPrestige) {
                        prestigeBtn.disabled = true;
                        prestigeBtn.classList.add('disabled');
                        prestigeBtn.textContent = 'No Benefit Available';
                    } else {
                        prestigeBtn.disabled = false;
                        prestigeBtn.classList.remove('disabled');
                        prestigeBtn.textContent = 'Hold to Prestige (3 seconds)';
                    }
                }
                
                // Update or remove the "no benefit" message
                const noBenefitMsg = document.querySelector('#prestige-button + p');
                if (!canPrestige) {
                    if (!noBenefitMsg) {
                        const msg = document.createElement('p');
                        msg.style.cssText = 'color: var(--text-muted); margin-top: 12px; text-align: center; font-size: 14px;';
                        msg.textContent = !hasEnoughMessages ? 
                            `You need at least ${formatNumber(Number(lastPrestigeMessages), 2)} messages to prestige (you have ${formatNumber(totalMessages, 2)}).` : 
                            'You need more messages to gain a benefit from prestiging.';
                        prestigeBtn.parentNode.insertBefore(msg, prestigeBtn.nextSibling);
                    } else {
                        // Update existing message
                        noBenefitMsg.textContent = !hasEnoughMessages ? 
                            `You need at least ${formatNumber(Number(lastPrestigeMessages), 2)} messages to prestige (you have ${formatNumber(totalMessages, 2)}).` : 
                            'You need more messages to gain a benefit from prestiging.';
                    }
                } else {
                ...
                if (currentMessagesEl) {
                    currentMessagesEl.textContent = formatNumber(totalMessages, 2);
                }
                if (currentBoostEl) {
                    currentBoostEl.textContent = `+${boostPercent}%`;
                }
                if (boostAfterEl) {
                    boostAfterEl.textContent = boostAfterPrestigeText;
                }
                
                // Update extra levels stat
                const extraLevelsEl = document.getElementById('prestige-extra-levels');
                const extraLevelsStat = extraLevelsEl?.closest('.upgrade-stat');
                if (extraLevels > 0) {
                    if (!extraLevelsStat) {
                        // Add the stat if it doesn't exist
                        const statsContainer = document.querySelector('.upgrade-stats');
                        if (statsContainer) {
                            const newStat = document.createElement('div');
                            newStat.className = 'upgrade-stat';
                            newStat.innerHTML = `
                                <span class="stat-label">Extra Global Upgrade Levels:</span>
                                <span class="stat-value" id="prestige-extra-levels">+${formatNumber(extraLevels, 0)}</span>
                            `;
                    ...
                    } else if (extraLevelsEl) {
                        extraLevelsEl.textContent = `+${formatNumber(extraLevels, 0)}`;
                    }
                } else {
                    // Remove the stat if no extra levels
                    if (extraLevelsStat) {
                        extraLevelsStat.remove();
                    }
                }
            }
        }, 1000);
        ...
        // Remove any existing DM mobile button
        const existingDMMobileButton = document.getElementById('dm-mobile-generate-button');
        if (existingDMMobileButton) {
            existingDMMobileButton.remove();
        }
        
        // Create and add mobile generate button for DMs if setting is enabled and on mobile
        const useMobileButton = gameState.settings.useMobileButton !== false; // Default to true
        if (useMobileButton && window.innerWidth <= 768) {
            const dmMobileButton = document.createElement('button');
            dmMobileButton.className = 'mobile-generate-button dm-mobile-button';
            dmMobileButton.id = 'dm-mobile-generate-button';
            dmMobileButton.innerHTML = `
                <svg class="mobile-generate-icon" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/>
                </svg>
                <span class="mobile-generate-label">Type</span>
            `;
            if (hasReachedMax) {
                dmMobileButton.disabled = true;
                dmMobileButton.classList.add('disabled');
            }
            document.body.appendChild(dmMobileButton);
        }
        
        // Setup DM input tracking
        setupDMInput();
        
        // Setup DM mobile button handlers
        const dmMobileButton = document.getElementById('dm-mobile-generate-button');
        if (dmMobileButton) {
            const maxChars = 5000;
            const counter = document.getElementById('dm-counter');
            
            // Function to add a character
            const addDMCharacter = () => {
                if (gameState.dmCharacters < maxChars) {
                    gameState.dmCharacters = (gameState.dmCharacters || 0) + 1;
                    const capped = Math.min(gameState.dmCharacters, maxChars);
                    if (counter) {
            ...
            // Generate on tap
            dmMobileButton.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (!isHolding && gameState.dmCharacters < maxChars) {
                    addDMCharacter();
                }
            });
            
            // Start hold detection
            dmMobileButton.addEventListener('touchstart', (e) => {
                e.preventDefault();
                e.stopPropagation();
                isHolding = false;
                
                // Add character immediately on touch
                if (gameState.dmCharacters < maxChars) {
                    addDMCharacter();
                }
                
                // After 300ms, start continuous generation
                holdTimeout = setTimeout(() => {
                    isHolding = true;
                    dmMobileButton.classList.add('holding');
                    
                    // Add character immediately when hold starts
                    if (gameState.dmCharacters < maxChars) {
                        addDMCharacter();
                    }
                    
                    // Current delay value (will change periodically)
                    let currentDelay = 20 + Math.random() * 5;
                    let charactersAtCurrentRate = 0;
                    const rateChangeInterval = 30; // Change rate every 30 characters
                    
                    // Recursive function to add characters with variable rate (40-50 per second)
                    const addWithVariance = () => {
                        if (!isHolding || gameState.dmCharacters >= maxChars) {
                            if (gameState.dmCharacters >= maxChars) {
                                dmMobileButton.disabled = true;
                                dmMobileButton.classList.add('disabled');
                        ...
                        addDMCharacter();
                        charactersAtCurrentRate++;
                        
                        // Change rate periodically (every ~30 characters)
                        if (charactersAtCurrentRate >= rateChangeInterval) {
                            currentDelay = 20 + Math.random() * 5;
                            charactersAtCurrentRate = 0;
                        }
                        
                        holdTimeout = setTimeout(addWithVariance, currentDelay);
                    };
            ...
            const stopHold = (e) => {
                ...
                dmMobileButton.classList.remove('holding');
            };
            
            dmMobileButton.addEventListener('touchend', stopHold);
            dmMobileButton.addEventListener('touchcancel', stopHold);
            dmMobileButton.addEventListener('mouseup', stopHold);
            dmMobileButton.addEventListener('mouseleave', stopHold);
            
            // Also handle mousedown for desktop testing
            dmMobileButton.addEventListener('mousedown', (e) => {
                e.preventDefault();
                e.stopPropagation();
                isHolding = false;
    ...
    try {
        ...
        const stateToSave = JSON.parse(JSON.stringify(gameState, (key, value) => {
            if (value instanceof Decimal) {
                // For very large Decimal values, use scientific notation to save space
                const str = value.toString();
                // Compress numbers longer than 50 digits to prevent localStorage quota issues
                if (str.length > 50 && !str.includes('e')) {
                ...
                return value.toString(); // Decimal can be serialized as string
            }
            return value;
        }));
        ...
        if (e.name === 'QuotaExceededError') {
            ...
            // Try to save without the large Decimal values as a fallback
            try {
                ...
                console.warn('Saved with compressed Decimal values. Some precision may be lost on very large numbers.');
            } catch (e2) {
...
function loadGameState() {
    ...
    if (saved) {
        try {
            const parsed = JSON.parse(saved);
            // Convert Decimal strings back to Decimal (handles both 'n' suffix and scientific notation)
            const parseDecimalValue = (value) => {
                ...
                if (typeof value === 'string') {
                    ...
                    if (value.startsWith('sci:')) {
                        ...
                        return new Decimal(sciPart);
                    }
                    // Check if it ends with 'n' (legacy BigInt format - convert to Decimal)
                    if (value.endsWith('n')) {
                        return new Decimal(value.slice(0, -1));
                    }
            ...
            if (parsed.messages !== undefined) {
                parsed.messages = parseDecimalValue(parsed.messages);
            }
            if (parsed.lifetimeMessages !== undefined) {
                parsed.lifetimeMessages = parseDecimalValue(parsed.lifetimeMessages);
            }
            if (parsed.generator1BotMessages !== undefined) {
                parsed.generator1BotMessages = parseDecimalValue(parsed.generator1BotMessages);
            }
            if (parsed.lastPrestigeMessageCount !== undefined) {
                parsed.lastPrestigeMessageCount = parseDecimalValue(parsed.lastPrestigeMessageCount);
            }
            ...
            if (gameState.lifetimeMessages === undefined) {
                gameState.lifetimeMessages = gameState.messages || new Decimal(0);
            }
            // Ensure Decimal fields are actually Decimal
            if (!(gameState.messages instanceof Decimal)) {
                gameState.messages = parseDecimalValue(gameState.messages);
            }
            if (!(gameState.lifetimeMessages instanceof Decimal)) {
                gameState.lifetimeMessages = parseDecimalValue(gameState.lifetimeMessages);
            }
            if (!(gameState.generator1BotMessages instanceof Decimal)) {
                gameState.generator1BotMessages = parseDecimalValue(gameState.generator1BotMessages);
            }
            ...
            // Ensure prestigeBoost exists
            if (gameState.prestigeBoost === undefined) {
                gameState.prestigeBoost = 1.0;
            }
            // Ensure lastPrestigeMessageCount exists
            if (gameState.lastPrestigeMessageCount === undefined) {
                gameState.lastPrestigeMessageCount = new Decimal(0);
            }
            // Ensure lastPrestigeMessageCount is Decimal
            if (!(gameState.lastPrestigeMessageCount instanceof Decimal)) {
                gameState.lastPrestigeMessageCount = parseDecimalValue(gameState.lastPrestigeMessageCount);
            }
            ...
            // Assign names to generators if they don't have one (preserves existing names)
            ['generator1', 'generator2', 'generator3'].forEach(genId => {
...
function showUnlockPrompt(generatorId) {
    ...
    const previewName = generatorId === 'prestige' ? 'Prestige' : getPreviewServerName(generatorId);
...
// Base max levels for Global Upgrades (before prestige bonus)
const GLOBAL_UPGRADE_BASE_MAX_LEVELS = {
...
// Get effective max levels for Global Upgrades (includes prestige bonus)
// Prestige adds 10 levels per 1% boost (whole percentages only)
// Cost Efficiency is capped at 10 (50% reduction max)
function getGlobalUpgradeMaxLevels() {
    const prestigeBoost = getPrestigeBoost();
    // Calculate whole percentage points (decimals don't count)
    const wholePercentBoost = Math.floor((prestigeBoost - 1.0) * 100);
    // Add 10 levels per 1% boost
    const prestigeBonus = wholePercentBoost * 10;
    
    return {
        manualGenerationMultiplier: GLOBAL_UPGRADE_BASE_MAX_LEVELS.manualGenerationMultiplier + prestigeBonus,
        autoGenerationBoost: GLOBAL_UPGRADE_BASE_MAX_LEVELS.autoGenerationBoost + prestigeBonus,
        messageMultiplier: GLOBAL_UPGRADE_BASE_MAX_LEVELS.messageMultiplier + prestigeBonus,
        costEfficiency: GLOBAL_UPGRADE_BASE_MAX_LEVELS.costEfficiency // Capped at 10, no prestige bonus
    };
}

// Legacy constant - replaced with function calls, but kept for reference
// Use getGlobalUpgradeMaxLevels() instead
const GLOBAL_UPGRADE_MAX_LEVELS = GLOBAL_UPGRADE_BASE_MAX_LEVELS;
...
// Perform full game prestige
function performFullGamePrestige() {
    const totalMessages = getTotalMessagesAsNumber();
    
    // Calculate new prestige boost (linear: 1% per 1T messages, so 1T = +1% = 1.01x)
    const newBoost = getPrestigeBoostForMessages(totalMessages);
    
    // Handle very large boost values
    let actualNewBoost;
    if (newBoost && typeof newBoost === 'object' && newBoost.isVeryLarge) {
        // For very large values, use the multiplier directly
        // newBoost = 1 + multiplier, so we can calculate: currentBoost * (1 + multiplier)
        const newBoostValue = 1.0 + newBoost.multiplier;
        const currentBoost = gameState.prestigeBoost || 1.0;
        
        // Multiply existing boost by new boost (compounding)
        actualNewBoost = currentBoost * newBoostValue;
        
        // Check if result is too large
        if (!isFinite(actualNewBoost) || actualNewBoost === Infinity) {
            // Use Number.MAX_VALUE as approximation for extremely large values
            actualNewBoost = Number.MAX_VALUE;
        }
    } else {
        // Multiply existing boost by new boost (compounding)
        actualNewBoost = (gameState.prestigeBoost || 1.0) * newBoost;
    }
    
    gameState.prestigeBoost = actualNewBoost;
    
    // Store the message count at prestige time (to prevent prestiging with fewer messages)
    gameState.lastPrestigeMessageCount = new Decimal(totalMessages) || new Decimal(0);
    ...
    // Reset all generators and relock servers
    gameState.generators = {
        unlocked: [], // Relock all servers
        generator1: {
            bots: 0,
            efficiency: 1.0,
            botSpeed: 0,
    ...
    // DM progress and buffs stay (dmCharacters, dmRewardClaimed, etc. are not reset)
    ...
    autoSave();
    updateCurrencyDisplay();
    renderServerSidebar(); // Re-render sidebar to show locked servers
    
    // Reload current channel to show updated state
    if (currentServer === 'prestige' && currentChannel === 'prestige') {
        loadChannel('prestige');
    } else {
        // If not on prestige channel, go to home
        loadServer('home');
        loadChannel('manual');
    }
...
function setupManualGeneration() {
    ...
    const mobileButton = document.getElementById('mobile-generate-button');
    if (mobileButton) {
        let holdInterval = null;
        let holdTimeout = null;
        let isHolding = false;
        ...
        mobileButton.addEventListener('touchstart', (e) => {
            ...
            // Generate immediately on touch
            generateMessage();
            
            // After 300ms, start continuous generation
            holdTimeout = setTimeout(() => {
                isHolding = true;
                mobileButton.classList.add('holding');
                
                // Generate immediately when hold starts
                generateMessage();
                
                // Current delay value (will change periodically)
                let currentDelay = 20 + Math.random() * 5;
                let messagesAtCurrentRate = 0;
                const rateChangeInterval = 30; // Change rate every 30 messages (~0.6-0.75 seconds)
                
                // Recursive function to generate with variable rate (40-50 per second)
                const generateWithVariance = () => {
                    if (!isHolding) return;
                    
                    generateMessage();
                    messagesAtCurrentRate++;
                    
                    // Change rate periodically (every ~30 messages)
                    if (messagesAtCurrentRate >= rateChangeInterval) {
                        currentDelay = 20 + Math.random() * 5;
                        messagesAtCurrentRate = 0;
                    }
                    
                    holdTimeout = setTimeout(generateWithVariance, currentDelay);
                };
                
                // Start the recursive generation
                generateWithVariance();
            }, 300);
        }, { passive: false });
        ...
        const stopHold = (e) => {
            ...
            // Set isHolding to false first to stop recursive generation
            isHolding = false;
            ...
            mobileButton.classList.remove('holding');
        };
        
        mobileButton.addEventListener('touchend', stopHold);
        mobileButton.addEventListener('touchcancel', stopHold);
        mobileButton.addEventListener('mouseup', stopHold);
        mobileButton.addEventListener('mouseleave', stopHold);
        
        // Also handle mousedown for desktop testing
        mobileButton.addEventListener('mousedown', (e) => {
            e.preventDefault();
            e.stopPropagation();
            isHolding = false;
    ...
    // Focus the input when channel loads (only on desktop)
    if (window.innerWidth > 768) {
        setTimeout(() => {
...
function setupSettingsHandlers() {
    ...
    // Mobile input radio buttons
    const mobileInputRadios = document.querySelectorAll('input[name="mobileInput"]');
    mobileInputRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            if (e.target.checked) {
                gameState.settings.useMobileButton = e.target.value === 'button';
                saveSettings();
                autoSave(); // Save full game state immediately
                // Reload manual channel if currently on it to apply the change
                if (currentServer === 'home' && currentChannel === 'manual') {
                    loadChannel('manual');
                }
            }
        ...
        resetBtn.addEventListener('click', () => {
            if (confirm('Are you sure you want to reset your game? This cannot be undone!')) {
                ...
                // Reset game state
                        gameState.messages = new Decimal(0);
                gameState.fractionalMessages = 0;
                gameState.lifetimeMessages = new Decimal(0);
                gameState.playtime = 0;
                gameState.sessionStartTime = Date.now();
                ...
                // Reset prestige boost and last prestige message count
                gameState.prestigeBoost = 1.0;
                gameState.lastPrestigeMessageCount = new Decimal(0);
                ...
                // Explicitly ensure prestigeBoost is reset (in case it wasn't set above)
                gameState.prestigeBoost = 1.0;
                ...
                // Defer UI updates to prevent blocking the main thread
                setTimeout(() => {
                    ...
                    // Reload current channel to refresh UI (especially for stats/prestige channels)
                    if (currentServer === 'prestige' && currentChannel === 'prestige') {
                        loadChannel('prestige');
                    } else if (currentServer === 'home' && currentChannel === 'stats') {
                        loadChannel('stats');
                    } else {
                        loadChannel(currentChannel);
                    }
    ...
    if (debugAddMessagesBtn) {
        debugAddMessagesBtn.addEventListener('click', () => {
            gameState.messages = gameState.messages.plus(10000);
            autoSave();
            updateCurrencyDisplay();
            updateUpgradeButtonStates();
        });
    ...
    if (debugDoubleMessagesBtn) {
        debugDoubleMessagesBtn.addEventListener('click', () => {
            gameState.messages = gameState.messages.times(1000);
            autoSave();
            updateCurrencyDisplay();
            updateUpgradeButtonStates();
        });
    ...
    if (debugSetMessagesBtn && debugSetMessagesInput) {
        const parseNumberInput = (input) => {
            ...
            if (scientificMatch) {
                ...
                // Use Decimal for all cases - it handles large numbers natively
                try {
            ...
            // Handle regular number - use Decimal
            try {
                // Try parsing as Decimal (handles 'n' suffix for legacy BigInt format)
                if (trimmed.includes('n')) {
                    return new Decimal(trimmed.replace('n', ''));
                }
                return new Decimal(trimmed);
            } catch (e) {
                return null;
...
function purchaseManualGenerationUpgrade() {
    ...
    // Check max level limit
    const GLOBAL_MAX = getGlobalUpgradeMaxLevels();
    if (currentLevel >= GLOBAL_MAX.manualGenerationMultiplier) {
        return; // Already at max
    }
    
    const count = Math.min(bulkPurchaseAmount, GLOBAL_MAX.manualGenerationMultiplier - currentLevel);
    const totalCost = getBulkManualGenerationUpgradeCost(currentLevel, count, GLOBAL_MAX.manualGenerationMultiplier);
    const totalMessages = getTotalMessagesAsNumber();
...
function purchaseAutoGenerationBoost() {
    ...
    // Check max level limit
    const GLOBAL_MAX = getGlobalUpgradeMaxLevels();
    if (currentLevel >= GLOBAL_MAX.autoGenerationBoost) {
        return; // Already at max
    }
    
    const count = Math.min(bulkPurchaseAmount, GLOBAL_MAX.autoGenerationBoost - currentLevel);
    const totalCost = getBulkAutoGenerationBoostCost(currentLevel, count, GLOBAL_MAX.autoGenerationBoost);
    const totalMessages = getTotalMessagesAsNumber();
...
function purchaseMessageMultiplier() {
    ...
    // Check max level limit
    const GLOBAL_MAX = getGlobalUpgradeMaxLevels();
    if (currentLevel >= GLOBAL_MAX.messageMultiplier) {
        return; // Already at max
    }
    
    const count = Math.min(bulkPurchaseAmount, GLOBAL_MAX.messageMultiplier - currentLevel);
    const totalCost = getBulkMessageMultiplierCost(currentLevel, count, GLOBAL_MAX.messageMultiplier);
    const totalMessages = getTotalMessagesAsNumber();
...
function purchaseCostEfficiency() {
    ...
    // Check max level limit
    const GLOBAL_MAX = getGlobalUpgradeMaxLevels();
    if (currentLevel >= GLOBAL_MAX.costEfficiency) {
        return; // Already at max
    }
    
    const count = Math.min(bulkPurchaseAmount, GLOBAL_MAX.costEfficiency - currentLevel);
    const totalCost = getBulkCostEfficiencyCost(currentLevel, count, GLOBAL_MAX.costEfficiency);
    const totalMessages = getTotalMessagesAsNumber();
...
const ACHIEVEMENTS = [
    ...
    { id: 'maxed_out', name: 'Maxed Out', description: 'Max out any upgrade', category: 'upgrade', check: () => {
        ...
        const globalMax = getGlobalUpgradeMaxLevels();
    ...
    { id: 'prestige_boost_10', name: 'Prestige Power', description: 'Reach +10% prestige boost', category: 'prestige', check: () => (gameState.prestigeBoost || 1.0) >= 1.1 },
    { id: 'prestige_boost_50', name: 'Prestige Mastery', description: 'Reach +50% prestige boost', category: 'prestige', check: () => (gameState.prestigeBoost || 1.0) >= 1.5 },
    { id: 'prestige_boost_100', name: 'Prestige Legend', description: 'Reach +100% prestige boost', category: 'prestige', check: () => (gameState.prestigeBoost || 1.0) >= 2.0 },
    { id: 'prestige_boost_200', name: 'Prestige God', description: 'Reach +200% prestige boost', category: 'prestige', check: () => (gameState.prestigeBoost || 1.0) >= 3.0 },
    { id: 'prestige_boost_500', name: 'Prestige Deity', description: 'Reach +500% prestige boost', category: 'prestige', check: () => (gameState.prestigeBoost || 1.0) >= 6.0 },
    { id: 'prestige_boost_1000', name: 'Prestige Transcendence', description: 'Reach +1000% prestige boost', category: 'prestige', check: () => (gameState.prestigeBoost || 1.0) >= 11.0 },
    ...
    Object.values(categories).forEach(category => {
        ...
        category.achievements.forEach(achievement => {
            ...
            if (achievement.target !== undefined) {
                const formattedTarget = formatNumber(new Decimal(achievement.target), 2);
...
function renderChangelog() {
    ...
    const changelog = [
        ...
        },
        {
            ...
            changes: [
                'Switched from BigInt to break_infinity.js Decimal library for better precision and performance with very large numbers',
                'All message counts, calculations, and displays now use Decimal for improved accuracy',
                'Fixed decimal display in message counters - fractional messages now display correctly',
                'Improved number formatting to properly handle decimal places and remove trailing zeros',
                'Save/load system updated to support Decimal with backward compatibility for BigInt saves'
            ]
        },
        {
            version: '1.0.0',
            date: '2025-12-10', // Release date
            changes: [
                'Added full game prestige system - reset all progress for permanent production boost',
                'Prestige boost: 1% per 1T messages (linear scaling, compounds when prestiging)',
                'Prestige server unlocks after Generator 3 (cost: 1,000,000 messages)',
                'Prestige resets messages, upgrades, research, and generators but keeps DM progress and buffs',
                'Prestige boost displayed in stats channel and tracked for achievements',
                'Added 7 prestige boost achievements (10%, 50%, 100%, 200%, 500%, 1000% thresholds)',
                'Server pings now respect bulk purchase amount - only show when you can afford the selected bulk quantity',
                'Mobile menu button now shows total ping count across all servers',
                'Improved bulk purchase selector design - now uses accent color background with transparent overlay',
