...
let gameState = {
    ...
    lastPrestigeMessageCount: 0n // Message count at the time of last prestige (BigInt to handle large numbers)
};
...
function loadChannel(channelId) {
    ...
    if (channelId === 'manual' && currentServer === 'home') {
        ...
        const lastPrestigeMessages = gameState.lastPrestigeMessageCount || 0n;
        const totalMessagesBigInt = BigInt(Math.floor(totalMessages)) || 0n;
        const hasEnoughMessages = totalMessagesBigInt >= lastPrestigeMessages;
        ...
        if (prestigeBtn) {
            ...
            prestigeBtn.addEventListener('touchstart', startHold);
    ...
    if (saved) {
        try {
            ...
            if (parsed.lastPrestigeMessageCount !== undefined) {
                parsed.lastPrestigeMessageCount = parseBigIntValue(parsed.lastPrestigeMessageCount);
            }
            ...
            if (gameState.lastPrestigeMessageCount === undefined) {
                gameState.lastPrestigeMessageCount = 0n;
            }
            // Ensure lastPrestigeMessageCount is BigInt
            if (typeof gameState.lastPrestigeMessageCount !== 'bigint') {
                gameState.lastPrestigeMessageCount = BigInt(gameState.lastPrestigeMessageCount || 0);
            }
...
function performFullGamePrestige() {
    ...
    gameState.lastPrestigeMessageCount = BigInt(totalMessages) || 0n;
